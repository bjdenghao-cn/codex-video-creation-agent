...
已把当前 8 个 VID 工作流整理到同一个页面，包含 VID-001 至 VID-007（含两套 VID-006）的触发口令、时长、画幅、用途、素材合同和配图说明：

**[打开 VID Skill 全景目录](docs/vid-skills.md)**

公开目录已去除公司内部生图/视频依赖：图片统一使用 Codex 内置 `imagegen`，视频统一使用使用者已配置并有权限的 Seedance 2.5 入口。本机现有 Skill 和私有生产素材不会被该公开页面改写或上传。
